...
/*
 * Common Node Header Layout (nodeType, isRoot, parentPointer)
 */
...
/*
 * Leaf Node Header Layout (leafNodeNumCells)
 */
...
/*
 * Leaf Node Body Layout (leafNodeNumCells)
 */
...
/*
    In a split we need to distribute N+1 cells between two nodes (N original cells plus one new one)
*/
...
/*
    Until we start recycling new pages, new pages will always 
...
void create_new_root(Table* table, uint32_t right_child_page_num) {
    ...
    Address of right root page to contain the new root node. 
...
void internal_node_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num) {
    ...
    /*
    Because we store the rightmost child pointer separately from 
    the rest of the child/key pairs, we have to handle things 
    differently if the new child is going to become the rightmost child.
    */
...
void internal_node_split_and_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num) {
    ...
    if (splitting_root) {
        ...
        // update old_node to point to the new root's left child
...
void leaf_node_split_and_insert(Cursor* cursor, uint32_t key, Row* value) {
   ...
   for (int32_t i = LEAF_NODE_MAX_CELLS; i >= 0; i--) {
    ...
    // Update cell count for both leaf nodes
